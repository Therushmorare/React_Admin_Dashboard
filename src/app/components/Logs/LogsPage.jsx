...
const LogsPage = () => {
  ...
  const fetchLogs = async () => {
    try {
      ...
      // Transform backend logs → UI logs
  ...
  const mapType = (applicantType) => {
    ...
    if (applicantType.toLowerCase().includes("admin"))
  ...
  const mapSeverity = (action) => {
    ...
    if (lower.includes("failed") || lower.includes("error"))
      return "error";
    if (lower.includes("added") || lower.includes("approved"))
      return "success";
    if (lower.includes("warning")) return "warning";
    return "info";
  };

  const extractAction = (action) => {
    return action.split(":")[0];
  };

  const extractResource = (action) => {
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = `system_logs_${new Date()
      .toISOString()
      .split("T")[0]}.csv`;
    a.click();
  };
